			...
			if self.CurrentLength <= DYNAMIC_SEGMENT_LIMIT then
				...
				if self.AttachmentPart and self.Attachments then
					...
					if self.CurrentLength > 1 and self.Beams then
						...
						beam.Attachment0 = self.Attachments[self.CurrentLength - 1]
				...
				-- Just increase the length counter without creating segments
				-- These will be "virtual" segments that exist in data but not visually
			end
		end
	end
...
function AISnake:setConfidenceBuff()
	...
	-- if self.HeadParts and self.HeadParts.headOutline then
	--	head.HeadParts.headOutline.Color3 = Color3.fromRGB(255, 255, 0)
...
function AISnake:updateMovement(dt)
	...
	-- Head is now visible like OptimizedSnakeSystem (removed forced invisibility)
	...
	-- Adaptive segment update skip based on snake length
	local segmentSkip = 1
	...
	-- Update head attachment position
	...
	for i = 1 + updateOffset, maxSegmentToUpdate, segmentSkip do
		...
		if segment and segment.Parent then
			...
			if targetData then
				...
				-- Update attachment position for this segment
...
-- Add a simple brain update counter
...
-- Periodic cleanup of invalid snakes
local lastCleanup = 0
...
AISnake._brainConnection = RunService.Stepped:Connect(function(time, deltaTime)
	...
	-- Periodic cleanup of dead/invalid snakes
	...
	for i = 1, BRAIN_UPDATES_PER_FRAME do
		...
		if AISnake._brainUpdateIndex > #snakes or AISnake._brainUpdateIndex < 1 then
			AISnake._brainUpdateIndex = 1
		end

		local snake = snakes[AISnake._brainUpdateIndex]
		if snake and snake._active then
			-- Only update AI snakes near players
			...
			if nearestPlayerPos and snake.Position then
				...
				if dist > AI_UPDATE_DISTANCE then
					shouldUpdate = false
					-- Still do minimal position update for far snakes
...
function AISnake:getSegmentColor(index)
	-- For AI snakes, we don't have rainbow mode, but we need the color logic
	if index == 0 then
...
function AISnake:getSegmentSize(index, baseSize)
	...
	-- Add growth pulse effect when growing (if we implement growth animation)
	-- For now, we'll skip this since AI doesn't have isGrowing state
...
function AISnake:getBeamWidth(index, baseSize)
	...
	-- For AI snakes, use slightly smaller beam width to prevent showing outside
	local aiConfig = self.Config.AI or {}
...
function AISnake:updateSegmentVisibility(cameraPosition)
	...
	for i = 0, self.CurrentLength do
		local segment = self.Segments[i]
		if segment then
			-- Make sure segment is in workspace
			if not segment.Parent then
				segment.Parent = self.SegmentFolder
			end
			
			-- Calculate segment distance from camera
	...
	-- Update eye visibility based on camera distance
	if self.HeadParts then
		local headDistance = (self.HeadParts.head.Position - cameraPosition).Magnitude
		local eyeTransparency = headDistance > 600 and 1 or 0
...
function AISnake:syncBeamVisibility()
	-- Update ALL beams to match their segments
	for i = 0, self.CurrentLength - 1 do
		local beam = self.Beams[i]
		local seg1 = self.Segments[i]
		local seg2 = self.Segments[i + 1]
		
		if beam and beam.Parent and seg1 and seg2 then
			-- Make sure segments are in workspace
	...
	for key, beam in pairs(self.Beams) do
		if type(key) == "string" and beam and beam.Parent then
			if string.find(key, "overlap") then
				...
				if index and index < self.CurrentLength - 2 then
					local seg1 = self.Segments[index]
