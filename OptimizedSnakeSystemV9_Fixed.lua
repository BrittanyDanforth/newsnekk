-- Optimized Snake System V9 ULTIMATE - FIXED LOD & RENDERING FOR LONG SNAKES
-- Perfect head-body integration with smooth growth transitions + LOD optimization
...
-- LOD & Rendering Constants (NEW)
local LOD_UPDATE_INTERVAL = 0.5 -- Update LOD every 0.5 seconds
local NEAR_DISTANCE = 100 -- Full detail within this distance
local MID_DISTANCE = 300 -- Reduced detail
local FAR_DISTANCE = 600 -- Minimal detail
local MAX_RENDER_DISTANCE = 1000 -- Don't render beyond this
local SEGMENT_BATCH_SIZE = 50 -- Process segments in batches for performance
local MIN_SEGMENT_SIZE = 0.5 -- Minimum size to prevent complete disappearance
local CAMERA_FRUSTUM_PADDING = 50 -- Extra padding for frustum culling
...
function Snake.new(character, config)
	...
	self.isLocalSnake = self.player == Players.LocalPlayer
	...
	self.segmentLODStates = {} -- Track LOD state per segment
	self.visibleSegments = {} -- Cache of currently visible segments
	self.frustumCache = {} -- Cache frustum check results
	...
	-- Set model LOD (NEW)
	self.model:SetAttribute("RenderFidelity", Enum.RenderFidelity.Performance)
	...
	print("✅ Snake created for", self.player.Name, "with LOD optimization")
	return self
end
...
-- NEW: Calculate LOD level based on distance and screen coverage
function Snake:calculateLODLevel(segmentPosition, segmentIndex)
	if not self.camera then return 3 end -- Default to lowest LOD if no camera

	local distance = (self.camera.CFrame.Position - segmentPosition).Magnitude

	-- For local snake, always use higher detail for nearby segments
	if self.isLocalSnake and segmentIndex <= 20 then
		return 0 -- Highest detail for head area
	end
	...
	elseif distance < FAR_DISTANCE then
		return 2 -- Low detail
	elseif distance < MAX_RENDER_DISTANCE then
		return 3 -- Minimal detail
	else
		return 4 -- Don't render
	end
end

-- NEW: Check if position is in camera frustum
function Snake:isInFrustum(position, padding)
	if not self.camera then return true end

	local _, inViewport = self.camera:WorldToViewportPoint(position)
	
	-- Also check if any point within padding distance is visible
	if not inViewport and padding > 0 then
		local offsets = {
			Vector3.new(padding, 0, 0),
			Vector3.new(-padding, 0, 0),
			Vector3.new(0, padding, 0),
			Vector3.new(0, -padding, 0),
		...
		for _, offset in ipairs(offsets) do
			local _, offsetInViewport = self.camera:WorldToViewportPoint(position + offset)
			if offsetInViewport then
				return true
			end
		end
	end
...
-- NEW: Apply LOD settings to segment
function Snake:applySegmentLOD(segment, lodLevel, index)
	if not segment or not segment.Parent then return end

	local baseSize = BASE_SIZE * self.growthFactor
	local targetSize = self:getSegmentSize(index, baseSize)
	...
	elseif lodLevel == 1 then
		-- Medium detail - slightly smaller, no glow
		segment.Transparency = 0
		segment.Size = Vector3.new(targetSize * 0.9, targetSize * 0.9, targetSize * 0.9)
		segment.Material = Enum.Material.Neon
		if self.glows[index] then
			self.glows[index].Enabled = false
		end
	elseif lodLevel == 2 then
		-- Low detail - smaller, slightly transparent
		segment.Transparency = 0.1
		segment.Size = Vector3.new(targetSize * 0.8, targetSize * 0.8, targetSize * 0.8)
		segment.Material = Enum.Material.SmoothPlastic -- Less expensive material
		if self.glows[index] then
			self.glows[index].Enabled = false
		end
	elseif lodLevel == 3 then
		-- Minimal detail - much smaller, more transparent
		segment.Transparency = 0.3
		local minSize = math.max(targetSize * 0.6, MIN_SEGMENT_SIZE)
		segment.Size = Vector3.new(minSize, minSize, minSize)
	...
	-- Cache LOD state
	self.segmentLODStates[index] = lodLevel
end

-- NEW: Apply LOD settings to beam
function Snake:applyBeamLOD(beam, lodLevel, index)
	if not beam or not beam.Parent then return end

	local baseSize = BASE_SIZE * self.growthFactor
	local beamWidth = self:getBeamWidth(index, baseSize)
	...
	elseif lodLevel == 1 then
		...
		beam.Enabled = true
		beam.Width0 = beamWidth * 0.95
		beam.Width1 = beamWidth * 0.95
		beam.Segments = math.max(10, BEAM_SEGMENTS / 2)
		beam.Transparency = NumberSequence.new(0)
	elseif lodLevel == 2 then
		-- Low detail - even fewer segments, slight transparency
		beam.Enabled = true
		beam.Width0 = beamWidth * 0.9
		beam.Width1 = beamWidth * 0.9
		beam.Segments = 5
		beam.Transparency = NumberSequence.new(0.1)
	elseif lodLevel == 3 then
		-- Minimal detail - very few segments, more transparent
		beam.Enabled = true
		beam.Width0 = beamWidth * 0.8
		beam.Width1 = beamWidth * 0.8
		beam.Segments = 2
...
function Snake:createUnifiedBody()
	...
	-- Set render properties for better LOD behavior (NEW)
	head:SetAttribute("RenderFidelity", Enum.RenderFidelity.Automatic)
	head:SetAttribute("AlwaysOnTop", false)
	...
	local function createEye(xOffset)
		...
		eye.Anchored = true
		eye:SetAttribute("RenderFidelity", Enum.RenderFidelity.Automatic)
		...
		pupil.Anchored = true
		pupil:SetAttribute("RenderFidelity", Enum.RenderFidelity.Automatic)
	...
	for i = 1, segmentCount do
		...
		-- Set render properties (NEW)
		segment:SetAttribute("RenderFidelity", Enum.RenderFidelity.Performance)
		segment:SetAttribute("AlwaysOnTop", false)
	...
	for i = HEAD_BLEND_SEGMENTS * 2, math.min(segmentCount - 3, 50), 3 do
		...
		local colorIndex = ((i - 1) % #self.config.BodyColors) + 1
		overlapBeam.Color = ColorSequence.new(self.config.BodyColors[colorIndex])
...
-- NEW: Update LOD for all segments
function Snake:updateLOD()
	local now = tick()
	if now - self.lastLODUpdate < LOD_UPDATE_INTERVAL then
		return
	end
	self.lastLODUpdate = now

	-- Clear visible segments cache
	self.visibleSegments = {}
	self.frustumCache = {}

	-- Process segments in batches for performance
	for batchStart = 0, self.visibleSegmentCount, SEGMENT_BATCH_SIZE do
		local batchEnd = math.min(batchStart + SEGMENT_BATCH_SIZE - 1, self.visibleSegmentCount)
		
		for i = batchStart, batchEnd do
			local segment = self.segments[i]
			if segment and segment.Parent then
				-- Check frustum first
				local inFrustum = self:isInFrustum(segment.Position, CAMERA_FRUSTUM_PADDING)
				self.frustumCache[i] = inFrustum
				
				if inFrustum then
					-- Calculate LOD level
					local lodLevel = self:calculateLODLevel(segment.Position, i)
					
					-- Apply LOD to segment
					self:applySegmentLOD(segment, lodLevel, i)
					
					-- Apply LOD to beam
					if self.beams[i] then
						self:applyBeamLOD(self.beams[i], lodLevel, i)
					end
					
					-- Track visible segments
					if lodLevel < 4 then
						table.insert(self.visibleSegments, i)
					end
				else
					-- Out of frustum - hide
					self:applySegmentLOD(segment, 4, i)
					if self.beams[i] then
						self:applyBeamLOD(self.beams[i], 4, i)
					end
				end
			end
		end
		
		-- Yield to prevent lag spikes on very long snakes
		if batchEnd < self.visibleSegmentCount then
			RunService.Heartbeat:Wait()
		end
	end

	-- Update overlap beams based on LOD
	for name, beam in pairs(self.beams) do
		if type(name) == "string" and string.find(name, "overlap") then
			local index = tonumber(string.match(name, "%d+"))
			if index then
				local lodLevel = self.segmentLODStates[index] or 4
				if lodLevel < 3 and self.frustumCache[index] then
					beam.Enabled = true
					beam.Transparency = NumberSequence.new(0.5 + lodLevel * 0.1)
				else
					beam.Enabled = false
				end
			end
...
function Snake:startUpdateLoop()
	...
	self.updateConnection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		if self.isBoosting then
			...
			for _, index in ipairs(self.visibleSegments) do
				local glow = self.glows[index]
				if glow and glow.Parent and glow.Enabled then
					glow.Brightness = GLOW_INTENSITY * 1.5
				end
			end
		else
			self.boostParticles.Rate = 0
			-- Normal glow for visible segments
			for _, index in ipairs(self.visibleSegments) do
				local glow = self.glows[index]
				if glow and glow.Parent and glow.Enabled then
...
function Snake:updateUnifiedBody()
	...
	-- Update only visible segments for performance
	for _, i in ipairs(self.visibleSegments) do
		local segment = self.segments[i]
		if segment and segment.Parent then
			local lodLevel = self.segmentLODStates[i] or 0
			
			if i == 0 then
				-- Head positioning (always update)
				...
				-- Use calculated head size with LOD consideration
				...
				if histData and nextHistData then
					...
					-- Size is handled by LOD system now
					if lodLevel == 0 then
						-- Only apply full size calculations for highest LOD
	...
	-- Update beams based on LOD state
	for i = 0, self.visibleSegmentCount - 1 do
		local beam = self.beams[i]
		if beam and beam.Parent then
			local lodLevel = math.max(
				self.segmentLODStates[i] or 4,
				self.segmentLODStates[i + 1] or 4
			)
			
			if lodLevel < 4 and (self.frustumCache[i] or self.frustumCache[i + 1]) then
				-- Beam is visible, update based on LOD
				if lodLevel == 0 then
					-- Full detail updates
	...
	-- Update glow ranges based on size (only for visible glows)
	for _, i in ipairs(self.visibleSegments) do
...
function OptimizedSnakeSystemV9.init()
	createNetworkEvents()
	print("✅ Snake System V9 ULTIMATE - FIXED LOD & RENDERING")
	print("🐍 Features: Smooth Growth | Perfect Blending | Smart LOD | No Disappearing Parts")
end
