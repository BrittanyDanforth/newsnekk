-- Optimized Snake System V9 ULTIMATE - SEAMLESS UNIFIED RENDERING (FIXED GROWTH + LOD FIX)
-- Perfect head-body integration with smooth growth transitions + NO DISAPPEARING PARTS
...
-- LOD SYSTEM CONSTANTS (NEW - FIXES DISAPPEARING PARTS)
local LOD_UPDATE_INTERVAL = 0.5 -- How often to update LOD in seconds
local FORCE_RENDER_DISTANCE = 1000 -- Always render parts within this distance
local MAX_RENDER_DISTANCE = 4096 -- Roblox's absolute max streaming radius
local BEAM_LOD_MULTIPLIER = 1.5 -- Render beams further than parts
local MIN_SEGMENT_SIZE = 0.1 -- Minimum size to prevent complete invisibility
...
-- NEW: Force render properties on parts to prevent culling
local function applyAntiCullingProperties(part)
	-- These properties help prevent Roblox from culling the part
	part:SetAttribute("AlwaysRender", true)
	
	-- Use a large bounding box to prevent culling
	if part:IsA("BasePart") then
		-- Set render fidelity to maximum
		pcall(function()
			part.RenderFidelity = Enum.RenderFidelity.Precise
		end)
		
		-- Ensure part is always considered for rendering
		part.CanCollide = false
		part.CanQuery = false
		part.CanTouch = false
		part.Massless = true
		
		-- Force the part to be considered "important" by the engine
		part.Transparency = math.min(part.Transparency, 0.99) -- Never fully transparent
	end
end

-- NEW: LOD system to manage part visibility based on camera distance
	...
	function lodController:updateLOD()
		...
		for i = 0, snake.visibleSegmentCount do
			...
			if segment and segment.Parent then
				...
				-- Never cull head or early segments
				if i <= 10 or distance < FORCE_RENDER_DISTANCE then
					segment.Transparency = 0
					if snake.glows[i] then
						snake.glows[i].Enabled = true
					end
				else
					-- For local snake, keep all parts visible
					if self.isLocalSnake then
						segment.Transparency = 0
						if snake.glows[i] then
							snake.glows[i].Enabled = true
						end
					else
						-- For other snakes, use distance-based LOD
						if distance < MAX_RENDER_DISTANCE then
							...
							-- Don't make completely invisible, just very transparent
		...
		for name, beam in pairs(snake.beams) do
			if beam and beam.Parent then
				...
				if att0Pos and att1Pos then
					...
					-- Beams have longer render distance
					if self.isLocalSnake or distance < MAX_RENDER_DISTANCE * BEAM_LOD_MULTIPLIER then
						beam.Enabled = true
						-- Adjust transparency based on distance for non-local snakes
...
function Snake.new(character, config)
	...
	-- IMPORTANT: Set model streaming properties to prevent culling
	self.model.ModelStreamingMode = Enum.ModelStreamingMode.Persistent
	
	-- Create a large invisible part to act as the model's primary part
	-- This prevents the model from being culled when camera is far away
	...
	print("✅ Snake created for", self.player.Name, "with LOD protection")
	return self
end
...
function Snake:createUnifiedBody()
	...
	-- Create attachment holder part with anti-culling properties
...
function Snake:startUpdateLoop()
	...
	self.updateConnection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		-- Update LOD system to prevent part disappearing
		self.lodController:updateLOD()
		
		-- Update model's primary part position to keep it relevant
		...
		if beam and beam.Parent then
			if type(i) == "number" then
				...
				if i <= self.visibleSegmentCount then
					...
					beam.Width0 = beamWidth
					beam.Width1 = beamWidth
					-- Don't disable beams here - let LOD system handle it
...
function OptimizedSnakeSystemV9.init()
	...
	print("🐍 Features: Smooth Growth | Perfect Blending | No Disappearing Parts | Smart LOD")
end
