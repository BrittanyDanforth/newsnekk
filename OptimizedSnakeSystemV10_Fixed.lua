-- Optimized Snake System V10 ULTIMATE - FIXED VERSION
-- Fixed eyes disappearing and stray beam at spawn issues
...
function Snake:createUnifiedBody()
	...
	head.CanQuery = true
	head.Anchored = true
	head.CFrame = self.rootPart.CFrame
	head.Position = self.rootPart.Position
	...
	-- Eyes - Initialize with proper position
	local function createEye(xOffset)
		...
		eye.Anchored = true
		eye.CFrame = head.CFrame * CFrame.new(xOffset * 0.6, 0.3, -0.35) -- Initial position relative to head
		...
		pupil.Anchored = true
		pupil.CFrame = eye.CFrame * CFrame.new(0, 0, -0.15) -- Initial position relative to eye
	...
	for i = 1, segmentCount do
		...
		-- Initialize position based on history
		local spacing = BASE_SIZE * SEGMENT_SPACING
	...
	-- Update attachment positions before creating beams
	for i = 0, segmentCount do
	...
	-- Create beams in batches - FIXED to prevent stray beams
	local beamBatch = {}
	for i = 0, segmentCount - 1 do
		if self.segments[i] and self.segments[i + 1] and self.attachments[i] and self.attachments[i + 1] then
			...
			beam.Enabled = true -- Make sure beam is enabled
...
function Snake:updatePositionHistory()
	local now = tick()
	if now - self.lastHistoryUpdate < 1/120 then
		return
	end
...
function Snake:updateCriticalSegments()
	...
	-- Update eyes with proper sizing and positioning
	...
	-- Update eye sizes
	local eyeSize = Vector3.new(eyeScale, eyeScale, eyeScale)
	local pupilSize = Vector3.new(eyeScale * 0.5, eyeScale * 0.5, eyeScale * 0.5)
	
	self.leftEye.Size = eyeSize
	self.rightEye.Size = eyeSize
	self.leftPupil.Size = pupilSize
	self.rightPupil.Size = pupilSize
...
function Snake:updateBeamsBatched(currentBaseSize)
	...
	for i, beam in pairs(self.beams) do
		if beam and beam.Parent and type(i) == "number" then
			if i < self.visibleSegmentCount and self.segments[i] and self.segments[i + 1] then
...
function Snake:addSegmentsBatched(count)
	...
	for i = self.visibleSegmentCount + 1, self.visibleSegmentCount + count do
		...
		-- Create beam
		if self.attachments[i - 1] and self.segments[i - 1] then
			local beam = getPooledBeam()
			beam.Name = "Beam" .. (i - 1)
...
function OptimizedSnakeSystemV10.init()
	createNetworkEvents()
	print("✅ Snake System V10 ULTIMATE - FIXED VERSION!")
	print("🐍 Fixed: Eyes now properly update | No stray beams at spawn")
end
