...
-- Enhanced with LOD system for long snakes
...
-- LOD Constants (NEW)
local LOD_UPDATE_INTERVAL = 0.2 -- Update LOD every 0.2 seconds
local LOD_DISTANCE_NEAR = 100 -- Full quality within this distance
local LOD_DISTANCE_MID = 300 -- Medium quality
local LOD_DISTANCE_FAR = 600 -- Low quality
local LOD_SEGMENT_SKIP = { -- How many segments to skip rendering at each LOD
    NEAR = 1, -- Show every segment
    MID = 2,  -- Show every 2nd segment
    FAR = 4   -- Show every 4th segment
}
local LOD_BEAM_QUALITY = { -- Beam segment quality at each LOD
    NEAR = 25,
    MID = 15,
    FAR = 8
}
...
-- Rendering Constants (NEW)
local ALWAYS_RENDER_SEGMENTS = 30 -- Always render first N segments regardless of LOD
local RENDER_FIDELITY_AUTOMATIC = Enum.RenderFidelity.Automatic
local RENDER_FIDELITY_PERFORMANCE = Enum.RenderFidelity.Performance
local FORCE_RENDER_DISTANCE = 50 -- Force segments to render within this distance
...
function Snake.new(character, config)
    ...
    self.currentLOD = "NEAR"
    self.lodSegmentVisibility = {}
    ...
    self.model.ModelStreamingMode = Enum.ModelStreamingMode.Improved
...
function Snake:createUnifiedBody()
    ...
    -- Set render properties for head (NEW)
    head.RenderFidelity = RENDER_FIDELITY_AUTOMATIC
    ...
    local function createEye(xOffset)
        ...
        eye.Anchored = true
        eye.RenderFidelity = RENDER_FIDELITY_AUTOMATIC
        ...
        pupil.Anchored = true
        pupil.RenderFidelity = RENDER_FIDELITY_AUTOMATIC
    ...
    for i = 1, segmentCount do
        ...
        -- Set render properties based on segment index (NEW)
        if i <= ALWAYS_RENDER_SEGMENTS then
            segment.RenderFidelity = RENDER_FIDELITY_AUTOMATIC
        else
            segment.RenderFidelity = RENDER_FIDELITY_PERFORMANCE
        end
        ...
        -- Initialize LOD visibility tracking (NEW)
        self.lodSegmentVisibility[i] = true
...
function Snake:updateLOD()
    ...
    -- Update LOD for each segment
    for i = 1, self.visibleSegmentCount do
        local segment = self.segments[i]
        if segment and segment.Parent then
            local distance = (segment.Position - cameraPos).Magnitude
            
            -- Always render segments for local player or within force render distance
            if self.isLocalPlayer or distance <= FORCE_RENDER_DISTANCE or i <= ALWAYS_RENDER_SEGMENTS then
                self.lodSegmentVisibility[i] = true
                segment.Transparency = 0
                
                -- Update render fidelity
                segment.RenderFidelity = RENDER_FIDELITY_AUTOMATIC
                
                -- Ensure glow is visible
                if self.glows[i] then
                    self.glows[i].Enabled = true
                end
            else
                ...
                -- Determine visibility based on LOD and segment index
                local skipFactor = LOD_SEGMENT_SKIP[lodLevel] or 999
                local shouldShow = (i % skipFactor == 0) or (i <= ALWAYS_RENDER_SEGMENTS)
                
                if lodLevel == "HIDDEN" then
                    shouldShow = false
                end
                
                self.lodSegmentVisibility[i] = shouldShow
                segment.Transparency = shouldShow and 0 or 1
                
                -- Update render fidelity based on LOD
                if shouldShow then
                    segment.RenderFidelity = lodLevel == "NEAR" and RENDER_FIDELITY_AUTOMATIC or RENDER_FIDELITY_PERFORMANCE
                end
                
                -- Manage glow visibility
                if self.glows[i] then
                    self.glows[i].Enabled = shouldShow and (lodLevel ~= "FAR")
                end
            end
        end
    end
    
    -- Update beam quality based on average LOD
    local nearCount = 0
    local totalVisible = 0
    
    for i = 1, math.min(50, self.visibleSegmentCount) do
        if self.lodSegmentVisibility[i] then
            local segment = self.segments[i]
            if segment then
                local distance = (segment.Position - cameraPos).Magnitude
                if distance <= LOD_DISTANCE_NEAR then
    ...
    -- Determine overall LOD level
    local lodLevel = "FAR"
    if nearCount > totalVisible * 0.5 then
        lodLevel = "NEAR"
    elseif nearCount > totalVisible * 0.2 then
        lodLevel = "MID"
    end
    
    self.currentLOD = lodLevel
    
    -- Update beam segments based on LOD
    local beamQuality = LOD_BEAM_QUALITY[lodLevel] or LOD_BEAM_QUALITY.FAR
    for _, beam in pairs(self.beams) do
        if beam and beam.Parent then
            beam.Segments = beamQuality
        end
    end
end
...
function Snake:updateUnifiedBody()
    ...
    for i = 0, self.visibleSegmentCount do
        local segment = self.segments[i]
        if segment and segment.Parent then
            -- Check LOD visibility (NEW)
            local isVisible = (i == 0) or self.lodSegmentVisibility[i] or self.isLocalPlayer
            
            if not isVisible and i > ALWAYS_RENDER_SEGMENTS then
                -- Skip updating invisible segments for performance
                continue
            end
    ...
    -- Update all beams with calculated widths and LOD visibility
    for i, beam in pairs(self.beams) do
        if beam and beam.Parent then
            if type(i) == "number" then
                -- Regular beams
                if i <= self.visibleSegmentCount then
                    -- Check if both segments are visible (NEW)
                    local seg1Visible = (i == 0) or self.lodSegmentVisibility[i] or (i <= ALWAYS_RENDER_SEGMENTS)
                    local seg2Visible = self.lodSegmentVisibility[i + 1] or ((i + 1) <= ALWAYS_RENDER_SEGMENTS)
                    
                    -- Show beam if at least one segment is visible or it's a critical beam
                    local showBeam = self.isLocalPlayer or seg1Visible or seg2Visible or (i <= ALWAYS_RENDER_SEGMENTS)
            ...
            elseif string.find(i, "overlap") then
                -- Overlap beams - only show for visible segments
                local index = tonumber(string.match(i, "%d+"))
                if index and index <= self.visibleSegmentCount - 2 then
                    local showBeam = self.isLocalPlayer or self.lodSegmentVisibility[index] or (index <= ALWAYS_RENDER_SEGMENTS)
                    if showBeam then
...
function Snake:addSegments(count)
    for i = self.visibleSegmentCount + 1, self.visibleSegmentCount + count do
        ...
        if self.attachments[i - 1] then
            ...
            beam.CurveSize1 = 0
            beam.FaceCamera = true
            beam.Segments = LOD_BEAM_QUALITY[self.currentLOD] or BEAM_SEGMENTS
        ...
        if i <= HEAD_BLEND_SEGMENTS * 2 and i > 2 and i % 2 == 0 then
            ...
            overlapBeam.CurveSize1 = 0
            overlapBeam.FaceCamera = true
            overlapBeam.Segments = LOD_BEAM_QUALITY[self.currentLOD] or BEAM_SEGMENTS
            overlapBeam.Texture = "rbxasset://textures/ui/LuaChat/icons/ic-gift.png"
            overlapBeam.TextureMode = Enum.TextureMode.Stretch
            overlapBeam.TextureLength = 3
            overlapBeam.LightEmission = 0.7
...
function OptimizedSnakeSystemV9.init()
    createNetworkEvents()
    print("✅ Snake System V9 ULTIMATE - SEAMLESS UNIFIED RENDERING (FIXED LOD)")
    print("🐍 Features: Smooth Growth | Perfect Blending | No Teleporting | Optimized LOD")
end
