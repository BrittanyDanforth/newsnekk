...
local BEAM_WIDTH_BASE = 1.0 -- Base beam width relative to segments (increased for better coverage)
local BEAM_TAPER_STRENGTH = 0.12 -- How much beams taper (reduced for fuller appearance)
...
function Snake:createUnifiedBody()
	...
	for i = 0, math.min(segmentCount - 2, HEAD_BLEND_SEGMENTS * 2) do
		if i % 2 == 0 then -- Every other segment
			...
			local overlapWidth = self:getBeamWidth(i, BASE_SIZE) * 1.2 -- 20% wider for better gap coverage
			...
			overlapBeam.Transparency = NumberSequence.new(0.2) -- Less transparent for better visibility
	...
	for i = HEAD_BLEND_SEGMENTS * 2, math.min(segmentCount - 3, 50), 3 do
		...
		local overlapWidth = self:getBeamWidth(i, BASE_SIZE) * 1.15 -- Increased for better coverage
		overlapBeam.Width0 = overlapWidth
		overlapBeam.Width1 = overlapWidth
		overlapBeam.CurveSize0 = 0
		overlapBeam.CurveSize1 = 0
		...
		overlapBeam.LightInfluence = 0
		overlapBeam.Transparency = NumberSequence.new(0.3) -- Less transparent for better coverage
		...
		if beam and beam.Parent then
			...
			elseif string.find(i, "overlap") then
				...
				if index and index <= self.visibleSegmentCount - 2 then
					local overlapWidth = self:getBeamWidth(index, currentBaseSize) * 1.2
...
function Snake:addSegments(count)
	for i = self.visibleSegmentCount + 1, self.visibleSegmentCount + count do
		...
		if self.attachments[i - 1] then
			...
			beam.LightEmission = 1
			beam.LightInfluence = 0
			beam.Transparency = NumberSequence.new(0.3) -- Start less transparent for better visibility
			...
			transparencyConnection = RunService.Heartbeat:Connect(function()
				...
				-- Interpolate from 0.3 to 0 (less transparent to fully opaque)
				local transparency = 0.3 * (1 - progress)
		...
		if i <= HEAD_BLEND_SEGMENTS * 2 and i > 2 and i % 2 == 0 then
			...
			local overlapWidth = self:getBeamWidth(i - 2, BASE_SIZE * self.growthFactor) * 1.2
			overlapBeam.Width0 = overlapWidth
			overlapBeam.Width1 = overlapWidth
			overlapBeam.CurveSize0 = 0
			overlapBeam.CurveSize1 = 0
			...
			overlapBeam.LightEmission = 0.8
			overlapBeam.LightInfluence = 0
			overlapBeam.Transparency = NumberSequence.new(0.2) -- Less transparent
			overlapBeam.ZOffset = -0.1
...
function OptimizedSnakeSystemV9.init()
	...
	print("🐍 Features: Smooth Growth | Perfect Blending | No Teleporting")
end
