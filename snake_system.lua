-- Optimized Snake System V9 ULTIMATE - SEAMLESS UNIFIED RENDERING (FIXED)
-- Perfect head-body integration with smooth growth transitions
-- FIXED: Parts disappearing at distance - Force rendering all segments
...
-- Helper function to force parts to render at all distances
local function forceRenderPart(part)
	-- Set properties to prevent streaming/culling
	if part:IsA("BasePart") then
		-- Force the part to always render
		part:SetAttribute("RenderFidelity", Enum.RenderFidelity.Precise)
		
		-- If StreamingEnabled is on, mark as persistent
		pcall(function()
			if workspace.StreamingEnabled then
				part:SetAttribute("StreamingMode", "Persistent")
			end
		end)
		
		-- Additional properties to prevent culling
...
function Snake.new(character, config)
	...
	-- Mark the model to prevent streaming
	pcall(function()
		self.model:SetAttribute("StreamingMode", "Persistent")
	end)
...
function Snake:createUnifiedBody()
	...
	forceRenderPart(attachmentPart) -- Prevent streaming
	...
	forceRenderPart(head) -- CRITICAL: Prevent head from disappearing
	...
	local function createEye(xOffset)
		...
		forceRenderPart(eye) -- Prevent eyes from disappearing
		...
		forceRenderPart(pupil) -- Prevent pupils from disappearing
	...
	for i = 1, segmentCount do
		...
		forceRenderPart(segment) -- CRITICAL: Prevent segment from disappearing at distance
...
function Snake:addSegments(count)
	for i = self.visibleSegmentCount + 1, self.visibleSegmentCount + count do
		...
		segment.Parent = self.model
		forceRenderPart(segment) -- CRITICAL: Apply rendering fix to new segments
		self.segments[i] = segment
		...
		-- Collision tagging for new segments
		if i <= 50 then
			CollectionService:AddTag(segment, "SnakeSegment")
			segment:SetAttribute("SegmentIndex", i)
			segment:SetAttribute("OwnerName", self.player.Name)
...
function OptimizedSnakeSystemV9.init()
	createNetworkEvents()
	print("✅ Snake System V9 ULTIMATE - SEAMLESS UNIFIED RENDERING (FIXED)")
	print("🐍 Features: Smooth Growth | Perfect Blending | No Teleporting | Distance Rendering Fixed")
end
