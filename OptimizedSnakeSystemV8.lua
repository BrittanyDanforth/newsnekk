-- FOR -- Optimized Snake System V8 - ULTRA SMOOTH VISUAL BEAST
...
local MAX_BEAMS = 200 -- Increased beam limit for better tail coverage
...
function Snake.new(character, config)
	...
	-- For smooth orb eating animations
	self.eatingOrbs = {}
...
function Snake:createBody()
	...
	-- Create smooth beams between segments - INCREASED LIMIT!
	for i = 1, math.min(segmentCount, MAX_BEAMS) do -- Increased beam limit
...
-- New function for smooth orb eating animation
function Snake:eatOrb(orbPosition, orbColor, orbSize)
	-- Create a visual orb that will travel along the body
	...
	eatingOrb.Size = Vector3.new(orbSize or 1.5, orbSize or 1.5, orbSize or 1.5)
	eatingOrb.CanCollide = false
	eatingOrb.Anchored = true
	eatingOrb.Parent = self.model
	
	-- Add glow to eating orb
	...
	table.insert(self.eatingOrbs, {
		part = eatingOrb,
		startTime = tick(),
		duration = 2, -- 2 seconds to travel down body
		startPos = orbPosition,
		glow = orbGlow
	})
	
	-- Create eat effect at orb position
	...
	eatEffect.Size = Vector3.new(orbSize * 2, orbSize * 2, orbSize * 2)
	eatEffect.Transparency = 0.5
...
function Snake:startUpdateLoop()
	...
	self.updateConnection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		self:updateHead()
		self:updateBody()
		self:updateEatingOrbs() -- New function for orb animations
...
-- New function to update eating orbs
	...
	for i = #self.eatingOrbs, 1, -1 do
		...
		if progress >= 1 then
			...
			if self.segments[segmentIndex] then
				...
				-- Smooth movement with slight oscillation
				local oscillation = math.sin(elapsed * 10) * 0.2
				orbData.part.Position = targetPos + Vector3.new(oscillation, oscillation, oscillation)
				
				-- Fade out near end
				if progress > 0.8 then
					local fadeProgress = (progress - 0.8) / 0.2
					orbData.part.Transparency = fadeProgress
					orbData.glow.Brightness = 2 * (1 - fadeProgress)
				end
				
				-- Scale down as it travels
				local scaleFactor = 1 - progress * 0.5
				orbData.part.Size = Vector3.new(1.5, 1.5, 1.5) * scaleFactor
			end
		end
	end
end
...
function Snake:updateBody()
	...
	-- Update beam widths and appearance - INCLUDING TAIL BEAMS!
	local beamCount = math.min(#self.beams, self.visibleSegmentCount)
	for i = 1, beamCount do
		...
		if beam and beam.Parent then
			...
			-- Less aggressive tapering for better tail visibility
			width = width * (1 - progress * 0.2) -- Only 20% taper instead of 30%
...
function Snake:addSegments(count)
	for i = self.visibleSegmentCount + 1, self.visibleSegmentCount + count do
		...
		-- Create beam if within INCREASED limit
		if i <= MAX_BEAMS and self.attachments[i - 1] then
			...
			beam.LightInfluence = 0
			beam.Transparency = NumberSequence.new(0) -- COMPLETELY SOLID - NO TRANSPARENCY!
			beam.Enabled = true -- Ensure it's enabled
	...
	-- Update final attachment for last beam
	if self.attachments[self.visibleSegmentCount + 1] then
		self.attachments[self.visibleSegmentCount + 1].Parent = self.attachmentPart
	else
		local finalAttachment = Instance.new("Attachment")
		finalAttachment.Name = "AttachmentFinal"
...
function OptimizedSnakeSystemV8.init()
	...
	print("🐍 Features: Dynamic Growth | Smooth Movement | Zero Lag | Head-Body Connection Fixed | Smooth Orb Eating")
end
