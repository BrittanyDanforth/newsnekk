...
-- ENHANCED: Smooth turning mechanics integrated from PR #1098
...
local SEGMENT_SPACING = 0.7 -- Even tighter spacing for ultra-smooth look (was 0.8)
...
-- Smoothing Constants (NEW from PR #1098)
local POSITION_SMOOTHING = 0.4 -- Increased from 0.3 for smoother movement
local CURVE_SMOOTHING_FACTOR = 0.3 -- For weighted curve smoothing
local TURNING_SMOOTHNESS = 0.95 -- For smoother turns
...
-- ENHANCED: Position history with interpolation for smoother curves
function Snake:updatePositionHistory()
	...
	if distance > 0.1 then
		-- Interpolate between positions for smoother curves
		local steps = math.min(math.floor(distance / 0.5), 3) -- More interpolation steps
...
-- NEW: Smooth position with curve enhancement using Catmull-Rom splines
function Snake:getSmoothPosition(stepsBack)
	-- Get surrounding history points for smoothing
	local p1 = self:getHistoricalPosition(math.max(0, stepsBack - 2))
	...
	local t = 0.5 -- Middle of the segment
	local t2 = t * t
	local t3 = t2 * t
	
	-- Catmull-Rom coefficients
	local tension = 0.5
	local v0 = (p3.position - p1.position) * tension
	local v1 = (p4.position - p2.position) * tension
	
	local a = p2.position
	local b = v0
	local c = (p3.position - p2.position) * 3 - v0 * 2 - v1
	local d = (p2.position - p3.position) * 2 + v0 + v1
	
	-- Calculate smoothed position
	local smoothedPos = a + b * t + c * t2 + d * t3
	
	-- Also smooth the look vector
	local smoothedLook = p2.lookVector:Lerp(p3.lookVector, t).Unit
	
	return {
		position = smoothedPos,
		lookVector = smoothedLook,
		time = p3.time
	}
end
...
-- ENHANCED: Update body with smooth turning mechanics
function Snake:updateOptimizedBody()
	...
	for i = 1, MAX_PHYSICAL_SEGMENTS do
		...
		if segment and segment.Parent then
			-- Calculate position from history with smoothing
			local distanceFromHead = i * spacing
			local stepsBack = math.floor(distanceFromHead / 2)
			local histData = self:getSmoothPosition(stepsBack) -- Use smooth position

			if histData then
				-- Enhanced smooth position update with curve preservation
				local targetPos = histData.position
				local currentPos = segment.Position
				
				-- Additional smoothing for better curves
				if i > 1 then
					local prevSegment = self.physicalSegments[i - 1]
					if prevSegment and prevSegment.Parent then
						local prevPos = prevSegment.Position
						local idealDir = (currentPos - prevPos).Unit
						local targetDir = (targetPos - prevPos).Unit
						
						-- Smooth direction change to prevent sharp turns
						if idealDir.Magnitude > 0.1 and targetDir.Magnitude > 0.1 then
							local smoothDir = idealDir:Lerp(targetDir, TURNING_SMOOTHNESS).Unit
							local adjustedPos = prevPos + smoothDir * spacing
							targetPos = targetPos:Lerp(adjustedPos, CURVE_SMOOTHING_FACTOR)
						end
					end
				end
				
				-- Apply position with enhanced smoothing
				segment.Position = currentPos:Lerp(targetPos, POSITION_SMOOTHING)
	...
	-- Update beam attachments positions with smoothing
	...
	for i = 1, requiredBeams + 1 do
		...
		if attachment then
			-- Calculate position along snake with enhanced smoothing
			local distanceFromHead = (i - 1) * BEAM_SEGMENT_LENGTH
			local stepsBack = math.floor(distanceFromHead / 2)
			local histData = self:getSmoothPosition(stepsBack) -- Use smooth position

			if histData then
				-- Apply cubic easing for even smoother attachment movement
				local currentWorldPos = attachment.WorldPosition
				local targetWorldPos = histData.position
				
				-- Smooth the attachment movement
				attachment.WorldPosition = currentWorldPos:Lerp(targetWorldPos, POSITION_SMOOTHING * 1.2)
			end
		end
	end
...
function OptimizedSnakeSystemV8.init()
	...
	print("🌀 SMOOTH TURNING: Catmull-Rom Splines | Enhanced Interpolation | No Square Turns!")
end
