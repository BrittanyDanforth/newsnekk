...
function Snake.new(character, config)
	...
	-- Pre-fill history with starting position (optimized)
	...
	for i = 1, HISTORY_SIZE do
		self.positionHistory[i] = historyEntry -- Share reference for initial state
	end
...
function Snake:createBody()
	...
	for i = 1, segmentCount do
		...
		segment.CanTouch = i <= COLLISION_SEGMENTS -- Optimized collision count
		...
		-- Glow for nearby segments (optimized)
		if i <= 10 then -- Reduced from 20
...
function Snake:getDistanceToCamera(position)
	if not self.camera then
		return 0
	end
	return (self.camera.CFrame.Position - position).Magnitude
end
...
function Snake:updateHead()
	...
	-- FORCE EYES TO BE SOLID!!!
	self.leftEye.Transparency = 0
	self.rightEye.Transparency = 0
	self.leftPupil.Transparency = 0
	self.rightPupil.Transparency = 0
...
function Snake:addSegments(count)
	for i = self.visibleSegmentCount + 1, self.visibleSegmentCount + count do
		...
		segment.CanCollide = false
		segment.CanTouch = i <= COLLISION_SEGMENTS
		segment.CanQuery = false
		segment.Anchored = true
...
function Snake:GetSegments()
	-- Return collision segments for optimization
	local collisionSegments = {}
	for i = 1, math.min(COLLISION_SEGMENTS, self.visibleSegmentCount) do
