...
local SEGMENT_SPACING = 0.7 -- Even tighter spacing to reduce gaps
...
local BEAM_MIN_WIDTH = 3 -- Increased to help cover gaps
local BEAM_MAX_WIDTH = 12 -- Slightly increased
...
function Snake:updateBody()
	...
	for i, beam in ipairs(self.beams) do
		if beam and beam.Parent and i <= self.visibleSegmentCount then
			...
			-- FORCE BEAMS TO BE COMPLETELY SOLID - NO TRANSPARENCY
			beam.Transparency = NumberSequence.new(0) -- SOLID ALWAYS
		end
	end
end
