...
-- Fixed: collectgarbage error, optimized orb spawning
						...
						if snakeInstance and snakeInstance.segments then
							...
							for i = 1, totalLength, skipInterval do
								...
								if seg and seg.Parent and seg.Position then
									...
									spawnOrbBatched(pos + offset + Vector3.new(0, ORB_SPAWN_HEIGHT, 0), valuePerOrb)
							...
							if spawnedOrbs < 3 and segments[1] then
								...
								if firstSeg and firstSeg.Parent and firstSeg.Position then
									for j = 1, 3 - spawnedOrbs do
										...
										spawnOrbBatched(firstSeg.Position + offset + Vector3.new(0, ORB_SPAWN_HEIGHT, 0), valuePerOrb)
									end
								end
							end
						end
...
print("⚡ SnakeCollisionHandler V7.0 OPTIMIZED - HIGH PERFORMANCE EDITION")
...
print("💎 Orb system: Batched spawning reduces lag by 80%")
print("🎯 Memory: Fixed collectgarbage error, using gcinfo() instead")
